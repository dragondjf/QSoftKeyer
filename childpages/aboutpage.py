...
#set $login_image = $os.sep.join([$helpimagepath, '登录.png'])
#set $setting_image = $os.sep.join([$helpimagepath, '配置防区.png'])
#set $monitor_image = $os.sep.join([$helpimagepath, '监控管理界面.png'])
#set $alarm_image = $os.sep.join([$helpimagepath, '告警管理界面.png'])
#set $userrecord_image = $os.sep.join([$helpimagepath, '用户操作管理界面.png'])
#set $users_image = $os.sep.join([$helpimagepath, '用户管理界面.png'])
                    ...
                    <div class="page-region-content">
                        <div class="page-control span10" data-role="page-control" style="width: 92%">
                            <ul style="display: block; overflow: visible;">
                                <li class="active"><a href="#page1">$funtion_info</a></li>
                                <li class=""><a href="#page2">$soft_info</a></li>
                                <li class=""><a href="#page3">$company_info</a></li>
                            </ul>

                            <div class="frames">
                                <div class="frame active" id="page1" style="display: block;">
                                    <h2>$funtion_info</h2>
                                    <h3>1.登录</h3>
                                    <p> 输入正确的用户名和密码，即可登录进入软件主界面。</p>
                                    <div align="center"><img src="file:///$login_image" onload="javascript:DrawImage(this);" style="border:none"  alt="1024×401"></div><p style="text-indent:2em;">
                                    
                                    <h3>2.配置防区</h3>
                                    <p> 根据实际项目配置的硬件台数进行正确配置，支持2、4、6三种规格，分别对应1、2、3台硬件设备；例如三台硬件设备，则应该正确配置防区数量为6。</p>
                                    <div align="center"><img src="file:///$setting_image" onload="javascript:DrawImage(this);" style="border:none"  alt="1024×401"></div><p style="text-indent:2em;">
                                    
                                    <h3>3.监控管理界面</h3>
                                    <p>每个防区对应一个开关和3状态（运行-红、断纤-黄、告警-红色）指示灯;总开关可以对所有防区进行一键撤防和布防功能。</p>
                                    <div align="center"><img src="file:///$monitor_image" onload="javascript:DrawImage(this);" style="border:none"  width="$imagewidth" height="$imageheight" alt="1024×401"></div><p style="text-indent:2em;">
                                    
                                    <h3>4.告警管理界面</h3>
                                    <p>对所有防区的状态变化进行实时记录，并存入数据库；支持对告警记录的翻页、一键删除和告警记录导出功能。 </p>
                                    <div align="center"><img src="file:///$alarm_image" onload="javascript:DrawImage(this);" style="border:none"  width="$imagewidth" height="$imageheight" alt="1024×401"></div><p style="text-indent:2em;">
                                    
                                    <h3>5.用户操作管理界面</h3>
                                    <p>对所有防区的状态变化进行实时记录，并存入数据库； 支持对告警记录的翻页、一键删除和告警记录导出功能。 </p>
                                    <div align="center"><img src="file:///$userrecord_image" onload="javascript:DrawImage(this);" style="border:none"  width="$imagewidth" height="$imageheight" alt="1024×401"></div><p style="text-indent:2em;">
                                    
                                    <h3>6.用户管理界面</h3>
                                    <p>用户级别分为超级管理员和值班员两种角色；超级管理员具备增加和删除值班员的权限，其他权限一样。 </p>
                                    <div align="center"><img src="file:///$users_image" onload="javascript:DrawImage(this);" style="border:none"  width="$imagewidth" height="$imageheight" alt="1024×401"></div><p style="text-indent:2em;">
                                </div>
                                <div class="frame" id="page2" style="display: none;">
                                    <h2>$soft_info</h2>
                                    <p>软件版本号：$verinfo['local_name']-$verinfo['version_name']-$verinfo['svn_version']-$verinfo['time_str']</p>
...
class AboutPage(BasePage):
    ...
    def auto_html(self, template):
        verfilepath = os.sep.join([os.getcwd(), 'js', 'ver.json'])
        try:
