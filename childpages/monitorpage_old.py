...
class MonitorPage(BasePage):
    ...
    def __init__(self, parent=None):
        ...
        # tipmainlayout = QtGui.QGridLayout()
        # tipmainlayout.addWidget(QtGui.QPushButton(u'11'), 10, 0)
        # self.tipwidget.setLayout(tipmainlayout)

        # mainLayout = QtGui.QGridLayout()
        # mainLayout.addWidget(self.tipwidget, 0, 0)
        # mainLayout.addWidget(self.tipwidget, 0, 9)
    ...
    def loadPAsfromWeb(self):
        ...
        if user == u'ov-orange' and password== u'ov-orange':
            ...
            try:
                ...
                logger.error(e)
                msg(u'请输入正确的防区列表链接地址(url)', msgoptions)
                self.loadPAsfromWeb()
            finally:
                if f:
                    f.close()
